...
pub(super) fn legend_element(
    ...
    // The legend prints at the terminal's own text size and FOLLOWS the Font slider in the
    // general settings (a fixed pixel size was tried and read as shouting on a laptop panel).
    let fs = f32::from(design::text_px(cx, 11.0));
