...
fn cluster<'a>(
    ...
    let chips: Vec<AnyElement> = slots
        .filter_map(|s| {
            chip(
                s.field, over, it, secs, share, decimals, view, badges, p, is_light, cx,
            )
        })
        .collect();
    if chips.is_empty() {
...
///
/// FORK: kept beside its one consumer (the card's windowed-turnover field) — the fork's own
/// volume band that used to own it was retired in favour of upstream's volumes.
fn format_quote_short(value: f32) -> String {
