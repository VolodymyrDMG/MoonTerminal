...
impl ChartEngine {
    ...
    /// Header Bv/Sv readout for one pane: buy and sell quote volume over the trailing
    /// `window_secs`, summed from the pane's volume tape against the wall clock.
    pub fn pane_volume_readout(&self, idx: usize, window_secs: u32) -> Option<(f32, f32)> {
        let mut state = self.state.borrow_mut();
        let now = moon_core::util::now_unix_ms();
        let pr = state.panes.get_mut(idx)?;
        if !pr.active {
            return None;
        }
        let from = now - f64::from(window_secs) * 1000.0;
        let (buy, sell) = pr.volume_tape.sum_window(from, f64::INFINITY);
        (buy > 0.0 || sell > 0.0).then_some((buy, sell))
    }
